...
| **Correctness model** | Probabilistic (failure prob <= negl(lambda)); inherited from base schemes [RMS24, Piano] |
...
| **What changed** | Prior OO-PIR schemes (Piano, RMS24) use standard PRFs to represent hint sets, requiring the client to perform a linear O-tilde(r) pass over all stored hints to find one containing the queried index. Plinko replaces PRFs with invertible PRFs (iPRFs), enabling O-tilde(1) hint searching and O-tilde(1) worst-case database updates. |
| **Superseded by** | N/A |
...
Prior single-server PIR schemes with client preprocessing (Piano, RMS24) achieve sub-linear query time t = O-tilde(r + n/r) where r is client storage, but always require O-tilde(r) client time to search for a relevant hint among the client's r stored hints.[^2] This means the total query time is O-tilde(r + n/r), which is only optimal when r = O(sqrt(n)). Plinko introduces a novel cryptographic primitive called an invertible pseudorandom function (iPRF) that allows the client to efficiently invert the PRF used for hint generation, finding all hints containing a given index x in O-tilde(1) time.[^3] This reduces total query time to O-tilde(n/r) for any choice of client storage r, matching the known lower bound r * t = Omega(n) up to polylogarithmic factors for all parameterizations.[^4] As a direct consequence, the iPRF inversion also enables worst-case O-tilde(1) database updates, since the client can immediately identify all hints affected by a changed entry.[^5]
...
[^4]: Abstract (p.1): "Our scheme uses t = O-tilde(n/r) query time for any client storage size r. This matches known lower bounds of r * t = Omega(n) up to logarithmic factors for all parameterizations."

[^5]: Abstract (p.1): "Plinko is also the first updateable PIR scheme where an entry can be updated in worst-case O-tilde(1) time."
...
| **Correctness definition** | For all y in R: Pr[F_k^{-1}(y) != {x in D : F_k(x) = y}] <= negl(lambda)[^8] |
| **Purpose** | Enable O-tilde(1) hint searching and O-tilde(1) worst-case database updates in OO-PIR schemes |
| **Built from** | PRP (pseudorandom permutation) + PMNS (pseudorandom multinomial sampler); both built from OWF[^9] |
| **Standalone complexity** | Gen: O-tilde(1); Forward evaluation F: O-tilde(1) time; Inversion F^{-1}: O(|pre-image|) time (linear in output size), with O(log m) calls to F and the PRP[^10] |
...
[^7]: Definition 4.1 (p.10): The adversarial advantage Adv^{iprf}_{iF}(lambda, A) is defined as the absolute difference between Pr[A^{F_k(.), F_k^{-1}(.)}(1^lambda) = 1] and Pr[A^{R(.), R^{-1}(.)}(1^lambda) = 1]. "We also observe that this definition is strictly stronger than the PRF security definition."

[^8]: Definition 4.1 (p.10): Correctness requires Pr[F_k^{-1}(y) != {x in D : F_k(x) = y}] <= negl(lambda) for all y in R.

[^9]: Theorem 4.4 (p.12): "Let P be a secure PRP over [n] and (Gen, S, S^{-1}) be a secure PMNS from [n] to [m]. Then, there exists a secure iPRF iF, defined as follows: iF.Gen(1^lambda) = (P.Gen(1^lambda), S.Gen(1^lambda)); iF.F((k1,k2),x) = S(k2, P(k1,x)); iF.F^{-1}((k1,k2),y) = {P^{-1}(k1,x) : x in S^{-1}(k2,y)}."
...
| **Correctness definition** | For all y in [m]: Pr[S^{-1}(k,y) != {x in [n] : S(k,x) = y}] <= negl(lambda) |
...
[^13]: Definition 4.3 (p.11): "A multinomial sampler MN(n,m) = (Gen, S, S^{-1}) is a pseudorandom multinomial sampler (PMNS) if, for all efficient A, Adv^{pmns}_{S}(lambda, A) <= negl(lambda)."
...
| **Correctness condition** | Pr[fail] <= negl(lambda); correctness is inherited from RMS24/Piano since iPRFs are computationally indistinguishable from random functions[^18] |
...
- **Regular hint table H:** lambda * w entries stored in H[0], ..., H[lambda*w - 1]. Each entry H[j] = (P_j, p_j) consists of a partition P_j (subset of c/2 + 1 blocks) and a parity p_j (XOR of selected entries).[^20]
- **Backup hint table T:** q entries stored in T[lambda*w], ..., T[lambda*w + q - 1]. Each entry T[j] = (B_j, ell_j, r_j) consists of a block subset B_j of size c/2, plus two parities ell_j and r_j (XOR of entries at iPRF-chosen offsets within B_j and its complement).[^21]
...
[^20]: Section 5.2 (p.21): "The regular hint table H is initialized with lambda*w regular hints that are stored in slots 0, ..., lambda*w - 1."

[^21]: Section 5.2 (p.21): "The backup table T is initialized with q backup hints that are stored in slots lambda*w, ..., lambda*w + q - 1."
...
| **UpdateHint** | Client | Invert iPRF at the updated index to find all O-tilde(1) hints containing x. XOR the update delta into each affected hint's parity. Also update Cache and hints under construction. | -- | Per DB mutation |
...
| **Failure probability** | Pr[fail] <= negl(lambda); each hint independently includes x with probability O(1/w), and there are lambda*w + q = O-tilde(r) hints total[^24] |
...
| **Key parameters affecting correctness** | lambda (security parameter), w (block size), q (backup count), r (client storage) |
| **Proof technique** | Chernoff bound on the number of hints containing any given entry. Expected number of hints containing x is O-tilde(r/w) = O-tilde(1). By Chernoff, at most O-tilde(1) hints contain x except with probability 2^{-lambda - log n}. Union bound over all n entries gives negligible failure probability.[^25] |
...
[^24]: Section 5.2, "Efficiency" (p.23): "As there are lambda*w + q = O-tilde(r) hints, we know the expected number of hints containing the x-th entry is O-tilde(r/w) = O-tilde(1)."

[^25]: Section 5.2, "Efficiency" (p.23): "By Chernoff's bound, we know that the x-th entry will not appear in more than max{O(lambda + log n), O-tilde(r/w)} except with probability 2^{-lambda - log n}."
...
|--------|-----------|---------------------------|-------|
| Query upload (communication) | O-tilde(n/r) bits[^26] | N/A (no implementation) | Online |
| Response (communication) | O-tilde(n/r) bits | N/A (no implementation) | Online |
| Server computation | O-tilde(n/r)[^27] | N/A (no implementation) | Online |
| Client computation (query) | O-tilde(n/r)[^28] | N/A (no implementation) | Online |
| Total query time (client + server) | O-tilde(n/r)[^29] | N/A (no implementation) | Online |

[^26]: Theorem 5.3 (p.20): "Each online query uses O-tilde(n/r) bits of communication."

[^27]: Theorem 5.3 (p.20): The server computes n/r parities, each requiring XOR of entries in one block.

[^28]: Section 5.2, "Efficiency" (p.23): "As the first step, the client searches for a hint containing the query index x using our iPRF construction... the client's query time becomes O-tilde(n/r) with the main cost coming from enumerating the n/r offsets."

[^29]: Theorem 5.3 (p.20): "Each online query runs in O-tilde(n/r) time."

### Preprocessing metrics

| Metric | Asymptotic | Concrete (benchmark params) | Phase |
|--------|-----------|---------------------------|-------|
| Client hint storage | O-tilde(r) bits[^30] | N/A (no implementation) | Persistent |
| Offline phase time | O-tilde(n)[^31] | N/A (no implementation) | Per window of q queries |
| Offline communication | O(n) bits (streaming download)[^32] | N/A (no implementation) | Per window of q queries |
| Amortization window | q queries (parameterizable; think of q = O-tilde(r))[^33] | N/A (no implementation) | -- |

[^30]: Theorem 5.3 (p.20): "The client uses O-tilde(r) memory."

[^31]: Theorem 5.3 (p.20): "The offline phase runs in O-tilde(n) time and uses O(n) communication."
...
[^34]: Section 5.2 (p.22): "More recent works [ZPZS24, RMS24] propose amortizing the offline phase by streaming n/q = O-tilde(n/r) database entries following each query... this does not increase query time or communication as each query already uses O-tilde(n/r) communication and time."

### Update metrics

| Metric | Value | Notes |
|--------|-------|-------|
| Cost per DB update (worst-case) | O-tilde(1) client time (polylog(n)) | Via single iPRF inversion to find all affected hints[^35] |
| Cost per DB update (amortized) | O-tilde(1) | Same as worst-case -- no amortization needed |
...
| **Impact on hints** | Each update affects O-tilde(1) hints in expectation and with high probability. Client identifies affected hints via iPRF inversion at the updated index.[^39] |
...
|---------|---------------|-------------|-------------------|---------------------|
| **Plinko (RMS24-based)** | Primary construction; modifies RMS24 by replacing PRFs with iPRFs | RMS24 | O-tilde(n/r) | O-tilde(1) |
| **Plinko-Piano** | Alternative construction applying iPRFs to Piano [ZPZS24] instead of RMS24 | Piano [ZPZS24] | O-tilde(n/r) | O-tilde(1) |
...
| **Tightness** | Plinko matches this bound: achieves t = O-tilde(n/r) for any r, giving r * t = O-tilde(n). Tight up to polylogarithmic factors for ALL parameterizations.[^42] |
...
[^42]: Section 1 (p.4): "Plinko obtains optimal query time t = O-tilde(n/r) for any choice of client storage size r."
...
|------------|-------------------|-------------|-------------|
| RMS24 [Group D] | Replace PRF with iPRF in hint generation and hint searching | Client query time reduced from O-tilde(r + n/r) to O-tilde(n/r); worst-case update reduced from O-tilde(sqrt(n)) to O-tilde(1) | Requires iPRF with small domain/range security (standard truncated PRPs insufficient)[^44] |
| Piano [ZPZS24, Group D] | Replace PRF with iPRF in hint generation and hint searching (Appendix B) | Client query time reduced from O-tilde(r + n/r) to O-tilde(n/r); worst-case update reduced from O(n) to O-tilde(1)[^45] | Same iPRF requirements |
...
| Server time | O-tilde(n^{1/3}) | O-tilde(n^{1/3}) | O-tilde(n^{1/3}) | -- |
| Client time | O-tilde(n^{1/3}) | O-tilde(n^{2/3}) | O-tilde(n^{2/3}) | -- |
| Total query time t | O-tilde(n^{1/3}) | O-tilde(n^{2/3}) | O-tilde(n^{2/3}) | Omega(n^{1/3}) |
| r * t | O-tilde(n) | O-tilde(n^{4/3}) | O-tilde(n^{4/3}) | Omega(n) |
| Crypto assumption | OWF | OWF | OWF | -- |

This example at r = n^{2/3} illustrates the gap that Plinko closes: prior work achieves total query time O-tilde(n^{2/3}) (dominated by the client's linear hint search), whereas the lower bound only requires Omega(n^{1/3}). Plinko matches the lower bound.[^46]

[^46]: Section 1 (p.4): "For example, when r = O(n^{2/3}), prior works require query times of t = O-tilde(n^{2/3}) whereas the lower bound specifies query time only need be as large as Omega(n^{1/3})."
...
**Key takeaway:** Plinko is the preferred scheme when (1) optimal space-time tradeoffs are needed for arbitrary client storage sizes (especially r >> sqrt(n)), and (2) worst-case O-tilde(1) database update efficiency is required. It achieves both properties using only one-way functions, at the cost of being theory-only with no implementation.

## Portable Optimizations

- **iPRF-based hint searching:** The technique of replacing PRFs with iPRFs for O-tilde(1) hint lookups is applicable to any OO-PIR scheme that uses PRF-compressed random sets (demonstrated on both RMS24 and Piano). Could potentially extend to other Group D schemes using similar hint structures.[^48]
...
- **Database updates:** Worst-case O-tilde(1) per update -- a significant advantage for mutable databases. Server sends O(log n) bits per mutation; client updates in-place.
...
- **Theory-only:** No implementation or benchmarks; practical constants hidden in O-tilde notation are unknown. The PMNS construction involves log m recursive binomial samples, each requiring a PRF evaluation and a binomial sampling step.
...
- **Small-domain iPRF requirement:** The iPRF must be secure even for small domains and ranges (the number of hints per block is approximately lambda * w, which may be comparable to or smaller than the block size w). Truncated PRPs are insufficient for this regime.[^51]
...
- **Notation for O-tilde:** The paper defines O-tilde to hide "multiplicative factors which are poly-logarithmic in the main variables" (p.9), treating lambda as constant. This differs from the standard convention where O-tilde hides polylog factors in the argument. The paper explicitly notes this deviation.
...
- **Parameter w vs r:** Throughout the paper, w (block size) and r (client storage) are treated as free parameters. The paper notes "it may be helpful to think of w = q = O-tilde(r)" (p.21), but the exact relationship depends on the target tradeoff point.
