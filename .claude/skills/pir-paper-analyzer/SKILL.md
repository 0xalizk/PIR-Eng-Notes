...
| **2a — Client-independent hint (download)** | §3.1 (often plain LWE) | §3.2, §3.3 | Client downloads server-computed global hint. Many use plain LWE — skip RLWE/GSW notation if so. DEPIR papers (BarelyDoublyEfficient) are theory/feasibility results. Includes FrodoPIR (plain LWE with global hint). |
| **2b — Client-specific hint (interactive)** | §3.2 (PRF/symmetric) | §3.1 (unless paper uses FHE) | Bidirectional hint generation, sublinear server. PRF-first; use §3.7b for Group 2b parameters. **Not all Group 2b schemes have probabilistic correctness** — check whether deterministic (SinglePass, WangRen) or probabilistic (Piano, RMS24, CK20). Includes ThorPIR (client-dependent FHE preprocessing). |
...
1. Locate the PDF: use `Glob` with `research/**/*.pdf` or a specific group path.
...
| **2a — Client-independent hint (download)** | Client downloads a server-computed global hint shared by all clients. Many use plain LWE. DEPIR papers (BarelyDoublyEfficient) achieve sublinear server computation — currently theoretical. | SimplePIR, DoublePIR, FrodoPIR, VeriSimplePIR, IncrementalPIR, BarelyDoublyEfficient |
| **2b — Client-specific hint (interactive)** | Bidirectional hint generation; each client gets a personalized offline hint. Sublinear server computation. Includes both single-server and two-server schemes. Not all Group 2b schemes have probabilistic correctness (SinglePass, WangRen are deterministic). Includes ThorPIR (client-dependent FHE preprocessing). | Piano, Plinko, TreePIR (2-server), CK20 (2-server+1-server), IncPIR (2-server), SinglePass (2-server), WangRen, IshaiShiWichs, RMS24, ThorPIR |
