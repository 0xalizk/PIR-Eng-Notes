...
<sub><nobr>9. <a href="#complexity"><b>Complexity</b></a></nobr></sub><br>
<sub><nobr>10. <a href="#performance-benchmarks"><b>Performance Benchmarks</b></a></nobr></sub>

</td><td>

<sub><nobr>11. <a href="#application-scenarios">Application Scenarios</a></nobr></sub><br>
<sub><nobr>12. <a href="#deployment-considerations">Deployment Considerations</a></nobr></sub><br>
<sub><nobr>13. <a href="#key-tradeoffs-limitations"><b>Key Tradeoffs & Limitations</b></a></nobr></sub><br>
...
<sub><nobr>18. <a href="#uncertainties">Uncertainties</a></nobr></sub><br>
<sub><nobr>19. <a href="#related-papers-in-collection">Related Papers in Collection</a></nobr></sub>
...
| Field | Value |
|-------|--------|
| **Builds on** | CK20 [Group 2b] (first sublinear-server single-server PIR in the client-preprocessing model); CHK22 (lower bound ST = Omega(n) for adaptive PIR without server preprocessing); BIM00 (linear server computation lower bound without preprocessing) |
...
| **Superseded by** | Plinko [Group 2b] (2024, invertible PRFs, worst-case O(1) updates); RMS24 [Group 2b] (2024, dummy subsets, standard correctness model) |
| **Concurrent work** | N/A |
...
| **Failure probability** | Bounded by negl(kappa) + negl(λ). With kappa = 40, the failure probability is bounded by 2^{-40} across all Q queries in a window, matching SimplePIR [Group 2a].&#8201;[^19] |
...
| Metric | Piano | SimplePIR [Group 2a] | CK20 [Group 2b] | CHK22 | ZLTS23/LP22 |
|--------|-------|--------------------|----|------|-------------|
...
- **PRP load balancing:** Applying a PRP to database indices converts arbitrary query patterns into random query patterns. This makes the balls-into-bins analysis exact and simplifies correctness proofs. Applicable to any Group 2b scheme with chunk-based structure.&#8201;[^45]
- **Pipelining preprocessing:** Running the next window's preprocessing concurrently with the current window's queries, at 2x storage cost, converts a bounded-query scheme into an unbounded-query scheme. Applicable to any Group 2b scheme with windowed amortization.
...
- **SimplePIR / DoublePIR [Group 2a]:** Primary comparison target. Piano achieves 43-915x faster online queries at the cost of client-dependent preprocessing and O(n) offline communication.
- **CK20 [Group 2b]:** Theoretical predecessor. First single-server sublinear PIR in the client-preprocessing model, but required puncturable PRFs and supported only a single query.
- **Plinko [Group 2b]:** Successor. Uses invertible PRFs for worst-case O(1) database updates, but is theory-only (no implementation).
- **TreePIR [Group 2b]:** Two-server concurrent work. Achieves polylogarithmic per-query communication via weak privately puncturable PRFs. For 2^28 entries (8 GB), TreePIR's best amortized online time is 23 ms (non-recursive) vs Piano's 8 ms amortized with 4x local storage.&#8201;[^51]
- **RMS24 [Group 2b]:** Successor. Uses dummy subsets for standard (non-consumable) correctness model.
