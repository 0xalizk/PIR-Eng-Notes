...
<sub><nobr>7. <a href="#protocol-phases-simplepir"><b>Protocol Phases — SimplePIR</b></a></nobr></sub><br>
<sub><nobr>8. <a href="#protocol-phases-doublepir"><b>Protocol Phases — DoublePIR</b></a></nobr></sub><br>
<sub><nobr>9. <a href="#correctness-analysis">Correctness Analysis</a></nobr></sub>
...
<sub><nobr>13. <a href="#implementation-notes"><b>Implementation Notes</b></a></nobr></sub><br>
<sub><nobr>14. <a href="#key-tradeoffs-limitations"><b>Key Tradeoffs & Limitations</b></a></nobr></sub><br>
<sub><nobr>15. <a href="#novel-primitives-abstractions">Novel Primitives / Abstractions</a></nobr></sub><br>
<sub><nobr>16. <a href="#application-certificate-transparency-auditing">Application: Certificate Transparency Auditing</a></nobr></sub><br>
<sub><nobr>17. <a href="#open-problems">Open Problems</a></nobr></sub><br>
<sub><nobr>18. <a href="#uncertainties">Uncertainties</a></nobr></sub>
...
<a id="protocol-phases-simplepir"></a>

### Protocol Phases — SimplePIR <a href="#toc">⤴</a>
...
<a id="protocol-phases-doublepir"></a>

### Protocol Phases — DoublePIR <a href="#toc">⤴</a>
...
#### Preprocessing metrics (Group 2a)
...
#### Client-side preprocessing (seed expansion)

The Init phase (labeled "Public" in both protocols) requires the client to expand A (or A_1, A_2) from a short seed via a PRG in counter mode (Section 4.1, p.7). This is non-trivial computation that the paper does not separately benchmark. The client needs the expanded matrix to generate queries: SimplePIR computes qu = A * s + e + Δ · u_{i_col}; DoublePIR computes c_1 = A_1 * s_1 + ... and c_2 = A_2 * s_2 + ....

The reported "< 1 ms" SimplePIR client time (Section 10, p.15) covers only the Recover step — O(n) key generation + inner product — and excludes query generation. DoublePIR's ~100 ms "query + recover" from Figure 18 (p.30) likely includes the A_1, A_2 seed expansion interleaved with the matrix-vector products.

**Analytical estimate of seed expansion cost (1 GB DB, 2^33 × 1-bit entries):**

| Scheme | Matrix dimensions | PRG output | Estimated time |
|--------|------------------|------------|---------------|
| SimplePIR | A: ~31K × 1024 | ~127 MB | ~50 ms |
| DoublePIR | A_1 + A_2: ~80K × 1024 | ~328 MB | ~130 ms |

Methodology: matrix element count derived from concrete query sizes (121 KB and 313 KB respectively, each element is 4 bytes in Z_{2^32}). PRG throughput estimated at ~2.5 GB/s for AES-CTR on the benchmark hardware (Intel Xeon Platinum 8000-series with AES-NI). If the implementation pre-expands A into memory (likely, given SimplePIR's "< 1 ms" per-query client time), this is a one-time cost amortized across all queries. If streamed per-query, it is subsumed into the per-query client time.
...
<a id="application-certificate-transparency-auditing"></a>

### Application: Certificate Transparency Auditing <a href="#toc">⤴</a>
...
- **Client preprocessing not separately benchmarked:** The paper does not report a standalone timing for client-side seed expansion (Init phase). SimplePIR's "< 1 ms" client time (Section 10, p.15) excludes query generation entirely (only Recover: O(n) inner product). DoublePIR's ~100 ms (Figure 18, p.30) is "query + recover" which likely includes seed expansion interleaved with A_1 * s_1, A_2 * s_2. Concrete estimates above (~50 ms SimplePIR, ~130 ms DoublePIR) are analytical, assuming ~2.5 GB/s PRG throughput on the benchmark hardware.
