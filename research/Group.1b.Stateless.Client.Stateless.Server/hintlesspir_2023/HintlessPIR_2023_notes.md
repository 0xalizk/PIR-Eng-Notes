...
| Field | Value |
|-------|--------|
| **Builds on** | SimplePIR / DoublePIR [Group 2a] (LWE-based PIR with client-independent hints); Spiral [Group 1a] (RLWE-based composition with Regev+GSW ciphertexts); Halevi-Shoup matrix-vector multiplication algorithm [31] |
...
| **Superseded by** | YPIR [Group 1b] (2024, eliminates offline communication via CDKS ring packing and GPU acceleration); later Group 1b schemes (WhisPIR, InsPIRe, VIA) build on similar stateless paradigms |
...
- **SimplePIR / DoublePIR [Group 2a]:** Direct ancestor. HintlessPIR replaces SimplePIR's hint download with a LinPIR query.
- **Spiral [Group 1a]:** Comparison point for RLWE-based PIR with compact communication but expensive computation.
- **YPIR [Group 1b]:** Successor that also eliminates offline communication, using CDKS ring packing and GPU acceleration for higher throughput.
- **FrodoPIR [Group 1a, functionally Group 2a]:** Essentially identical to SimplePIR (acknowledged by both papers); also requires database-dependent hint.
