...
| **PIR Category** | 2-server, role-asymmetric (offline/online split) — exploratory `multiserver/asymmetric.role/`. Conceptually closest to **Group 2b — Interactive-Hint** in the single-server taxonomy, but inherently 2-server. The online phase optionally recurses on a single-server √N PIR sub-protocol (e.g., Spiral, Döttling et al.) to drive bandwidth from √N down to polylog.&#8201;[^1] |
