...
[^14]: §1, §3.2, §4.1, §4.2 (pp. 2, 5, 6–8): consolidated discussion of the three deltas vs Riposte. §4.1: "our new auditing protocol … takes advantage of the structure of a particular DPF construction that is less efficient than the one used by Express. Applying their protocol to the more efficient DPFs used in Express would require client communication and computation `Ω(λn)` and `Ω(n)` respectively as well as the introduction of an additional non-colluding server."
