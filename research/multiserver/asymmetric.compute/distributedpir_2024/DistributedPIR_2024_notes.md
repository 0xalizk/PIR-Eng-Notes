...
[^11]: §4.1 *Identifying the bottleneck* (p. 5) and Table 1 (p. 5): for 2M messages, first PIR step costs 3990.26 ms vs 186.92 ms for the second PIR step and 23.57 ms for Freivalds verification — the first step is `~17×` the second and `~169×` verification.
...
[^31]: §7.2.2 Theorem 2 (p. 9): "the probability of the adversary corrupting a query and not being caught is upper-bounded by `1 − 2^{-d}` ... [equivalently catches with probability `≥ 1 − 2^{-d}`]."
