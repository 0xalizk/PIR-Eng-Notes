...
[^7]: §3.2 (p. 6): basic collision analysis gives `E[SuccessRate] ≈ m/n − ½(m/n)² + …`; "if we want an expected success rate of 95% then we need `n ≈ 19.5 m`." With `k=2`-way collision recovery (§3.2 p. 7): `E[SuccessRate] ≈ 1 − ½(m/n)² + ⅓(m/n)³`, yielding `n ≈ 2.7 m`.
