...
| **Correctness model** | Probabilistic (failure per-read <= 1/2, amplified to 2^{-lambda} via lambda parallel repetitions) |
...
- **Setup(1^lambda, n) -> (ck, q_h):** Client generates a client key ck and hint request q_h. Crucially, the client can run Setup *before* knowing which database bit to read.
...
**Correctness:** For every lambda, n, x in {0,1}^n, and i in [n], the probability that Reconstruct(h, a) = x_i is 1 (over all randomness).

**Security:** The scheme is *epsilon-secure* if for all i, j in [n], the query distributions D_{lambda,n,i} and D_{lambda,n,j} have distinguishing advantage at most epsilon(lambda, n). The scheme is *statistically secure* if this holds against computationally unbounded adversaries.[^5]
...
- **Setup(1^lambda, n) -> (ck, q_h):** As before.
...
| **Interface / Operations** | **Gen(1^lambda, n) -> sk:** Generate key for set of size s(n) over universe [n]. **Punc(sk, i) -> sk_p:** Puncture the key at element i in Eval(sk), producing a punctured key. **Eval(sk) -> S:** Expand the key into a set S of [n] with |S| = s(n). Also: **GenWith(1^lambda, n, i) -> sk:** Generate a key constrained to contain i. **Shift(sk, delta) -> sk':** Produce a key for the shifted set {x + delta mod n : x in Eval(sk)}.[^10] |
| **Security definition** | Game 1 (p.14): Given a punctured key sk_p, the adversary guesses the punctured element x*. The guessing advantage is PSAdv[A, Psi](lambda, n) := Pr[A wins] - 1/(n - s(n) + 1). A puncturable pseudorandom set is *computationally secure* if PSAdv <= negl(lambda), and *perfectly secure* if PSAdv = 0 for all unbounded adversaries.[^11] |
...
| **Built from** | Three constructions provided with increasing compactness: (1) Fact 2: Perfectly secure with linear-size keys (trivially store the set). (2) Theorem 3 / Construction 4: From puncturable PRFs (GGM-tree PRF from OWF), with keys of length O(lambda log n) and punctured keys of length O(lambda log n). (3) Theorem 7: From PRPs, with keys of length kappa(lambda, n) and punctured keys of length s * O(log n), plus fast membership test InSet.[^13] |
| **Standalone complexity** | Gen: O(s(n)) * poly(lambda, log n). Eval: O(s(n)) * poly(lambda, log n). Punc: O(s(n)) * poly(lambda, log n). InSet (PRP construction only): poly(lambda, log n).[^14] |
...
| **Interface / Operations** | **Gen(1^lambda, n) -> K_left = (k_1, ..., k_m):** Generate a family of left keys. **Choose(K_left, i) -> (j, k_right):** Given a left key family and index i in [n], choose a left key k_j and produce a right key k_right such that Eval(k_j) + Eval(k_right) = e_i. **Eval(k) -> v in F^n:** Evaluate a key into a vector.[^16] |
| **Security definition** | The right key k_right is computationally (or statistically) indistinguishable from a simulated key Sim_right(1^lambda, n), hiding the special index i.[^17] |
...
| **Correctness condition** | Pr[fail per single read] <= 1/2 (from Bernoulli sampling in Query). Amplified to 2^{-lambda} by running lambda parallel instances. Can be further transformed to *perfect correctness* at the cost of a negligible security loss.[^21] |
...
- **Client hint (single-query):** A set key sk in K, shift vector delta = (delta_1, ..., delta_m) in [n]^m, and parity bits h = (h_1, ..., h_m) in {0,1}^m, where m = (n/s) log n. Total client storage: O(lambda * kappa + (lambda * n / s(n)) * log^2 n) bits.[^22]
- **Client hint (multi-query):** m = (2n/s) log n independent set keys (sk_1, ..., sk_m) in K^m plus m parity bits. Total storage: O-tilde_lambda(n^{1/2}) bits with s = sqrt(n).[^23]

[^22]: Appendix C.2 (p.50): Client storage claim for the single-query scheme.
[^23]: Appendix D.2 (p.58): Client stores m = O-tilde(n^{1/2}) puncturable pseudorandom set keys.

### Variants

| Variant | Key Difference | Offline Comm | Online Comm | Online Server Time | Assumption | Ref |
|---------|---------------|-------------|-------------|-------------------|------------|-----|
| Two-server statistical (Thm 11) | Perfectly secure puncturable sets; statistical security | O(lambda * sqrt(n) * log^2 n) | O(lambda * sqrt(n) * log n) | O-tilde_lambda(sqrt(n)) | None (information-theoretic puncturable sets) | p.18 |
| Two-server computational (Thm 14) | PRG-based puncturable sets; computational security | O(lambda * sqrt(n) * log n) | O(lambda^2 * log n) | O-tilde_lambda(sqrt(n)) | PRG existence | p.19 |
| Two-server multi-query (Thm 17) | Supports poly-many adaptive queries from one offline phase | O(lambda * sqrt(n) * log n) | O(lambda * sqrt(n) * log n) | O-tilde_lambda(sqrt(n)) | PRP existence | p.22 |
| Two-server multi-query low-comm (Cor 18) | Reduced online communication via Hellman tables | O-tilde_lambda(n) | O(lambda^2 * log n) | O-tilde_lambda(sqrt(n)) | PRG existence | p.22 |
| Single-server LHE (Thm 20) | Uses linearly homomorphic encryption; no public-key ops online | O-tilde_lambda(n^{2/3}) | O-tilde_lambda(n^{1/3}) | O-tilde_lambda(n^{2/3}) | LHE + single-server PIR | p.25 |
| Single-server FHE (Thm 22) | Uses FHE to simulate both servers; optimal tradeoff | O-tilde_lambda(sqrt(n)) | O-tilde_lambda(sqrt(n)) | O-tilde_lambda(sqrt(n)) | FHE | p.27 |
...
|-------|-------|-----------|---------------|------------------|
| Setup | Client | Generate set key sk <- Gen(1^lambda, n); sample m random shifts delta_1, ..., delta_m <- [n]; set ck = (sk, delta_1, ..., delta_m) | -- | Per query (single-query variant) |
| Hint request | Client -> Offline server | Send set key sk and shifts delta | |q_h| = kappa + m * log n bits upward | Per query |
| Hint computation | Offline server | For j = 1, ..., m: compute S_j = Eval(Shift(sk, delta_j)); compute h_j = XOR of x_i for i in S_j | -- | Per query; server time: n * poly(lambda, log n) |
| Hint delivery | Offline server -> Client | Send h = (h_1, ..., h_m) | m bits downward | Per query |
| Query generation | Client | Find j such that i_pir - delta_j in Eval(sk). Sample b <- Bernoulli((s-1)/n). If b=0: i_punc = i_pir; else: i_punc = random from Eval(sk_q) \ {i_pir}. Send q = Punc(sk_q, i_punc) | lambda * kappa_p bits upward | Per query |
| Answer | Online server | Compute S* = Eval(q) (the punctured set, |S*| = s-1). Return a = XOR of x_i for i in S* | 1 bit (lambda bits for lambda repetitions) downward | Per query; server time: s(n) * poly(lambda, log n) |
...
| Phase | Actor | Operation | Communication | When / Frequency |
|-------|-------|-----------|---------------|------------------|
| Setup | Client | Generate m independent set keys sk_1, ..., sk_m <- Gen(1^lambda, n); set ck = (sk_1, ..., sk_m) | -- | Once per offline phase |
| Hint request | Client -> Left server | Send all m set keys | m * kappa bits upward | Once per offline phase |
| Hint computation | Left server | For j = 1, ..., m: compute S_j = Eval(sk_j); compute h_j = XOR of x_i for i in S_j | -- | Once; server time: O-tilde_lambda(n) |
| Hint delivery | Left server -> Client | Send h = (h_1, ..., h_m) | m bits downward | Once |
| Query (per read) | Client | Find j such that i in Eval(sk_j) (using InSet). Generate sk_new via GenWith(1^lambda, n, i). Swap sk_j with sk_new in ck. Sample b <- Bernoulli((s-1)/n). Compute q_left = Punc(sk_new, i_punc) and q_right = Punc(sk_right, i_punc) | lambda * kappa_p bits to each server | Per query |
| Answer (right server) | Right server | Expand Eval(q_right), compute parity of s-1 bits | 1 bit (lambda bits total) | Per query; server time: s * poly(lambda, log n) |
| Answer (left server) | Left server | Expand Eval(q_left), compute parity for hint refresh | 1 bit (lambda bits total) | Per query; server time: s * poly(lambda, log n) |
...
| **Failure probability** | Per single instance: <= 1/2. After lambda parallel repetitions: <= 2^{-lambda}. Multi-query: union bound over T reads gives failure <= T / 2^{lambda}.[^27] |
| **Probability grows over queries?** | No for single-query (each offline phase is independent). For multi-query (Construction 44), failure probability is bounded by T / 2^{lambda} where T is the number of queries, but per-query failure remains <= 1/2 per instance.[^28] |
| **Key parameters affecting correctness** | Set size s = s(n) = sqrt(n)/2 (from Construction 4); number of shifted sets m = (n/s) log n; number of parallel repetitions lambda |
...
| **Amplification** | Run lambda independent instances in parallel; client succeeds if any instance succeeds. Transforms 1/2 per-read failure to 2^{-lambda}.[^29] |
...
[^27]: Appendix C.2 (p.49-50): Amplification via lambda parallel repetitions.
[^28]: Appendix D.2 (p.57): Multi-query failure probability via union bound.
[^29]: Appendix C.2 (p.49-50): "By running lambda instances of the scheme in parallel, using independent randomness for each instance, we can drive the overall failure probability (when the puncturable pseudorandom set is perfectly secure) to 2^{-lambda}."
...
|--------|------|---------|-------|
| Offline server computation | O-tilde_lambda(n) | N/A (no implementation) | Offline (per-query for single-query; once for multi-query) |
| Offline communication (total) | O(lambda * sqrt(n) * log^2 n) [Thm 11] / O(lambda * sqrt(n) * log n) [Thm 14] | N/A (no implementation) | Offline |
| Online communication (total) | O(lambda * sqrt(n) * log n) [Thm 11] / O(lambda^2 * log n) [Thm 14] | N/A (no implementation) | Online |
| Client persistent storage | O(lambda * kappa + (lambda * n/s(n)) * log^2 n) bits | N/A (no implementation) | Between offline and online |
...
| **Client peak memory** | O-tilde_lambda(sqrt(n)) |
...
| **Proof technique** | Reduction to Yao's Box Problem (Definition 47, p.64): an offline/online PIR scheme implies an algorithm for Yao's Box Problem with C bits of advice and T queries. Applying the known lower bound for Yao's Box Problem (Theorem 48: epsilon = O-tilde(sqrt(C(T+1)/N))) completes the proof.[^34] |
| **Tightness** | Tight up to polylogarithmic factors. The two-server construction of Theorem 11 (C = O-tilde(sqrt(n)), T = O-tilde(sqrt(n))) achieves (C+1)(T+1) = O-tilde(n). The single-server FHE construction of Theorem 22 also matches.[^35] |
...
**Two-server (Theorem 14, s = sqrt(n)):** For n-bit database with security parameter lambda:
- Offline communication: O(lambda * sqrt(n) * log n) bits
- Online communication: O(lambda^2 * log n) bits
- Online server time: sqrt(n) * poly(lambda, log n)
- Client storage: O(lambda * kappa + lambda * sqrt(n) * log^2 n) bits, where kappa = O(lambda) (PRF key length)

**Remark 12 (Concrete efficiency, p.18):** The poly(lambda, log n) factors hidden in O-tilde notation can be made as small as O(lambda * log n) with careful implementation.[^37]

**Single-server LHE (Theorem 20):** For n-bit database:
- Offline: O-tilde_lambda(n^{2/3}) communication, O-tilde_lambda(n) server operations in group G
- Online: O-tilde_lambda(n^{1/3}) communication, O-tilde_lambda(n^{2/3}) time, zero group operations
- Client: O-tilde_lambda(n^{2/3}) time and memory

**Remark 13 (Tradeoff, p.18):** The communication-time tradeoff is continuous. For any function C(n) <= n/2, one can build a two-server scheme with C(n) bits of offline communication and O-tilde(n/C(n)) online server time.[^38]

[^37]: Remark 12 (p.18): "It is possible to make these hidden factors as small as O(lambda * log n)."
...
1. **Offline phase (unbalanced):** The client encrypts its set S as an indicator vector v in F^{sqrt(n)} using LHE: ct_v = Enc(k, v). The server computes ct_h = ct_v * Circ(x) via an FFT-like computation in O-tilde_lambda(n) time, exploiting the circulant structure. The client then uses batch PIR (IKOS04) to privately retrieve the m desired components of ct_h.

2. **Rebalancing:** The database is partitioned into n^{1/3} buckets of size n^{2/3} each. The unbalanced scheme is run once per bucket, with the client sending a single query per bucket. This yields total communication O-tilde_lambda(n^{2/3}) and online time O-tilde_lambda(n^{2/3}).[^40]

3. **With FHE (Theorem 22):** Using FHE instead of LHE, the client sends an encryption of the PRG seed rather than the set indicator vector. The server homomorphically evaluates the offline server's algorithm on the encrypted seed. This achieves the optimal O-tilde_lambda(sqrt(n)) communication and time, matching the lower bound.[^41]
...
All columns omit poly(lambda) factors and polylog(n) factors.[^42]
...
[^42]: Table 2 (p.6): Comparison table. "All columns omit poly(lambda) factors, for security parameter lambda, and also low-order polylog(n) factors."
...
- **Shift-based set derandomization:** The technique of generating multiple sets from a single base set via random shifts (Section 1.5, p.10) reduces communication from O(n log n) (sending explicit sets) to O-tilde(sqrt(n)), and is reusable in any scheme based on parity hints over random subsets.
...
- **Cold start suitability:** No -- requires offline phase with O-tilde(n) server computation before any query.
- **Amortization crossover (multi-query):** The multi-query scheme (Theorem 17) amortizes the linear offline computation over polynomially many queries. Each online query costs O-tilde(sqrt(n)), so the amortized per-query cost approaches O-tilde(sqrt(n)) after sufficiently many queries.

### Key Tradeoffs & Limitations

- **Theory-only:** No implementation or concrete parameter analysis. All bounds include unspecified poly(lambda, log n) factors.
...
- Remark 12 (p.18) states that the poly(lambda, log n) hidden factors "can be made as small as O(lambda log n)" but does not provide a worked example with concrete parameters.
