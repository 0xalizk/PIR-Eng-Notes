...
[^7]: Table 1 (p.2): Two-server: O(sqrt(N)) request, O(1) response, O(lambda*sqrt(N)) client storage, O(sqrt(N)) client computation, O(sqrt(N)) server computation. Single-server: O(sqrt(N)) request, O(sqrt(N)/lambda) response amortized, O(lambda*sqrt(N)) client storage.
...
| Single-server | Client streams entire DB offline; uses backup hints for replenishment | 1 | O(sqrt(N)/lambda) amortized | Via pre-computed backup hint pairs (Alg. 5) | 0.4 * lambda * sqrt(N) queries per offline phase[^9] |
...
[^9]: Section 3.4 (p.7): The client retrieves lambda*sqrt(N) main hints and lambda*sqrt(N) backup hints. Backup hints come in pairs. The client can make approximately 0.4*lambda*sqrt(N) queries before needing to re-run the offline phase (more precisely, 0.5*lambda*sqrt(N)/2 pairs of backup hints, and in expectation 0.5*lambda*sqrt(N) backup hints skip any given partition, so the client can make close to but fewer than 0.5*lambda*sqrt(N) queries).
...
| **Correctness condition** | Pr[fail] <= e^{-lambda/2} per query, by union bound over queries (Lemma 1 + Section 3.5)[^14] |
...
[^14]: Section 3.5 (p.8): Lemma 1 proves each hint contains any particular index with probability at least 1/(2*sqrt(N)). With M = lambda*sqrt(N) hints, the probability that none contains the queried index is at most (1 - 1/(2*sqrt(N)))^{lambda*sqrt(N)} < e^{-lambda/2}.
...
- **Client storage:** lambda*sqrt(N) main hints (two-server) or lambda*sqrt(N) main + lambda*sqrt(N)/2 backup pairs (single-server). Per hint: w + 64 bits (32-bit cutoff + 32-bit extra index + w-bit parity + 1 comparison bit).[^18]
...
|-------|-------|-----------|---------------|------------------|
| Offline (Alg. 1) | Offline server | Generate M = lambda*sqrt(N) hints using shared PRF key: compute cutoffs, select partitions, compute parities | O(lambda*sqrt(N)) words down to client[^19] | Once at setup |
...
[^19]: Section 3.6 (p.9): "The offline phase costs O(lambda*sqrt(N)) communication and O(lambda*N) computation at the offline server."
...
| Phase | Actor | Operation | Communication | When / Frequency |
|-------|-------|-----------|---------------|------------------|
| Streaming Offline (Alg. 4) | Client | Stream DB partition by partition; build 1.5M hints (M main + 0.5M backup pairs) using PRF key | N words down (full DB stream)[^21] | Every 0.4*lambda*sqrt(N) queries |
...
[^21]: Section 3.6 (p.9): "The streaming offline phase costs N communication and O(lambda*N) computation, and needs to be run every 0.5*lambda*sqrt(N) online queries." The single-server response overhead is O(sqrt(N)/lambda) amortized because the O(N) offline communication is amortized over 0.5*lambda*sqrt(N) queries.
...
| **Failure probability** | Pr[fail] <= (1 - 1/(2*sqrt(N)))^{lambda*sqrt(N)} < e^{-lambda/2}[^23] |
...
| **Key parameters affecting correctness** | Number of hints M = lambda*sqrt(N); coverage probability q = (sqrt(N)/2 + 1)/sqrt(N) * 1/sqrt(N) per hint per index |
| **Proof technique** | Lemma 1 gives per-hint coverage probability >= 1/(2*sqrt(N)); then apply (1-p)^M bound with M = lambda*sqrt(N). For subsequent queries, a distributional invariance argument shows replenished hints follow distribution R (identical to original).[^25] |
| **Amplification** | Not needed -- failure is already negligible for lambda = 80 (e^{-40}) |
| **Adaptive vs non-adaptive** | Correctness holds for fully adaptive queries -- this is the key distinction from Piano PIR[^26] |
| **Query model restrictions** | Two-server: unlimited queries. Single-server: bounded by backup hint supply (~0.4*lambda*sqrt(N) queries per offline phase). |
...
|--------|----------------------|---------------------------|-------------------------|------------------------------|
| Offline communication | O(lambda*sqrt(N)) words | 60.16 MB | O(N) words (full DB stream) | 8192 MB[^29] |
| Offline computation | O(lambda*N) | 842 s | O(lambda*N) | 1146 s |
| Client storage | O(lambda*sqrt(N)) words | 60.16 MB | O(lambda*sqrt(N)) words | 100 MB[^30] |
| Amortized response overhead | O(1) | 2x | O(sqrt(N)/lambda) | varies |
| Amortization window | Unlimited (constant) | -- | ~0.4*lambda*sqrt(N) queries | ~0.4 * 80 * 2^14 ~ 524K queries |
...
| **Client peak memory** | O(lambda*sqrt(N)) | O(lambda*sqrt(N)) |
| **Number of DB passes** | 1 (offline server) | 1 (streaming) |
| **Hint refresh mechanism** | Pipelining -- offline server replenishes on-the-fly after each query | Full re-download after ~0.4*lambda*sqrt(N) queries |

[^31]: Section 3.4 (p.7-8): Algorithm 4 streams the database one partition at a time. For each partition k, it downloads DB[k*sqrt(N) : (k+1)*sqrt(N) - 1] and processes all 1.5M hints to update their parities. This requires only O(lambda*sqrt(N)) client memory.
...
- **Dummy subset technique:** Applicable to any partition-based hint PIR scheme to eliminate leakage from the queried index's partition membership. Removes the need for parallel repetition (lambda-factor blowup on all costs). Could be applied to TreePIR's hint system.[^35]
...
- **Security parameter:** lambda = 80[^41]
...
[^41]: Section 4.1 (p.9): "We set the parameter lambda to 80."
...
- **Scalability concern:** Client storage is O(lambda*sqrt(N)), which reaches 660 MB at 64 GB database size (single-server). The paper acknowledges this as a limitation.[^46]
...
- **Client storage:** O(lambda*sqrt(N)) is sublinear but still large in practice -- 60 MB at 8 GB, 660 MB at 64 GB. Shared limitation with all amortized sublinear PIR schemes.
...
- **Reducing client storage:** The O(lambda*sqrt(N)) client storage is a limitation shared by all amortized sublinear PIR schemes.
...
|-------|-------|-------------|
| CK20 | Group D | First amortized sublinear PIR; RMS24 builds on the CK20 framework but eliminates the lambda parallel repetition factor and the O(N) client storage / O(N) per-query computation bottleneck |
...
- **Exact amortization window (single-server):** The paper states "close to, but fewer than, 0.5*lambda*sqrt(N)" queries. The "0.4*lambda*sqrt(N)" figure used in Table 1 caption is described as conservative ("say 0.4*lambda*sqrt(N)").
